...
DATA_DIR = get_dir('"..\..\game_data')
def load_data(filename=os.path.join(DATA_DIR, "game_states.json")):
    with open(filename) as f:    
      data = json.load(f)
    print('loading file %s' % (filename))
    return np.expand_dims(np.asarray(data), -1).reshape([-1, 5, 7, 1])
full_data = load_data()
train_data = full_data[:int(len(full_data) / 2)]
test_data = full_data[int(len(full_data) / 2):]
...
# TRAIN_DIR_'TRAIN_DIR_CLIPS'CLIPS = get_dir(os.path.join(DATA_DIR, '.Clips/'))
...
#                [3 * (HIST_LEN + 1), 128, 256, 512, 256, 128, 3]]
SCALE_FMS_G = [[1 * HIST_LEN, 1, 1, 1, 1],
               [1 * (HIST_LEN + 1), 1, 1, 1, 1],
               [1 * (HIST_LEN + 1), 1, 1, 1, 1, 1, 1],
               [1 * (HIST_LEN + 1), 1, 1, 1, 1, 1, 1]]
