...
class TestLPLoss:
    def test_same_images(self):
        ...
        for i in range(1, NUM_SCALES + 1):
            scale_preds.append(tf.constant(np.ones([BATCH_SIZE, 2**i, 2**i, 1])))
            scale_truths.append(tf.constant(np.ones([BATCH_SIZE, 2**i, 2**i, 1])))
    ...
    def test_opposite_images(self):
        ...
        res_tru = 0
        for i in range(1, NUM_SCALES + 1):
            scale_preds.append(tf.constant(np.zeros([BATCH_SIZE, 2**i, 2 ** i, 1])))
            scale_truths.append(tf.constant(np.ones([BATCH_SIZE, 2**i, 2 ** i, 1])))
...
class TestGDLLoss:
    def test_same_uniform(self):
        ...
        res_tru = 0
        for i in range(1, NUM_SCALES + 1):
            scale_preds.append(tf.ones([BATCH_SIZE, 2 ** i, 2 ** i, 1]))
            scale_truths.append(tf.ones([BATCH_SIZE, 2 ** i, 2 ** i, 1]))
    ...
    def test_same_nonuniform(self):
        ...
        for i in range(1, NUM_SCALES + 1):
            # generate batch of 3-deep identity matrices
            arr = np.empty([BATCH_SIZE, 2 ** i, 2 ** i, 1])
            imat = np.identity(2 ** i)
            for elt in range(BATCH_SIZE):
    ...
    def test_diff_uniform(self):
        ...
        res_tru = 0
        for i in range(1, NUM_SCALES + 1):
            scale_preds.append(tf.zeros([BATCH_SIZE, 2 ** i, 2 ** i, 1]))
            scale_truths.append(tf.ones([BATCH_SIZE, 2 ** i, 2 ** i, 1]))
    ...
    def test_diff_one_uniform_one_not(self):
        ...
        for i in range(1, NUM_SCALES + 1):
            ...
            preds = np.empty([BATCH_SIZE, 2 ** i, 2 ** i, 1])
            imat = np.identity(2 ** i) * 1
            for elt in range(BATCH_SIZE):
                preds[elt] = np.dstack([imat, imat, imat])

            scale_preds.append(tf.constant(preds, dtype=tf.float32))
            scale_truths.append(tf.zeros([BATCH_SIZE, 2 ** i, 2 ** i, 1]))
            ...
            # (# channels)
            num_diffs = BATCH_SIZE * 2 * 2 * 2**i * 1
